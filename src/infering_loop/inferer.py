...
from src.model.operator.cuda import HEAD_SIZE
...
# HEAD_SIZE = 64
load(name="wkv7s", sources=["src/model/cuda/wkv7s_op.cpp", f"src/model/cuda/wkv7s.cu"], is_python_module=False,
     verbose=True,
...
class RWKV_x070(MyModule):
    ...
    def __init__(self, args):
        ...
        z['emb.weight'] = F.layer_norm(z['emb.weight'], (args.n_embd,), weight=z['blocks.0.ln0.weight'],
    ...
    def forward(self, idx, state, full_output=False):
        if state == None:
            ...
            for i in range(self.args.n_layer):  # state: 0=att_x_prev 1=att_kv 2=ffn_x_prev
                ...
                state[i * 3 + 1] = torch.zeros((self.args.n_embd // self.args.head_size, self.args.head_size, self.args.head_size),
                                               dtype=torch.float, requires_grad=False, device="cuda")
    ...
    def forward_one(self, idx: int, state: List[torch.Tensor]):
        with torch.no_grad():
            ...
            for i in range(self.n_layer):
                ...
                xx, state[i * 3 + 0], state[i * 3 + 1], v_first = RWKV_x070_TMix_one(i, self.n_head, self.head_size, xx,
                                                                                     state[i * 3 + 0], v_first,
                                                                                     state[i * 3 + 1],
                                                                                     z[att + 'x_r'], z[att + 'x_w'],
                                                                                     z[att + 'x_k'], z[att + 'x_v'],
                ...
                xx, state[i * 3 + 2] = RWKV_x070_CMix_one(xx, state[i * 3 + 2], z[ffn + 'x_k'], z[ffn + 'key.weight'],
                                                          z[ffn + 'value.weight'])
                x = x + xx
    ...
    def forward_seq(self, idx: List[int], state: List[torch.Tensor], full_output: bool = False):
        with torch.no_grad():
            ...
            for i in range(self.n_layer):
                ...
                xx, state[i * 3 + 0], state[i * 3 + 1], v_first = RWKV_x070_TMix_seq(i, self.n_head, self.head_size, xx,
                                                                                     state[i * 3 + 0], v_first,
                                                                                     state[i * 3 + 1],
                                                                                     z[att + 'x_r'], z[att + 'x_w'],
                                                                                     z[att + 'x_k'], z[att + 'x_v'],
                ...
                xx, state[i * 3 + 2] = RWKV_x070_CMix_seq(xx, state[i * 3 + 2], z[ffn + 'x_k'], z[ffn + 'key.weight'],
                                                          z[ffn + 'value.weight'])
                x = x + xx
...
@MyStatic
def RWKV_x070_TMix_one(layer_id: int, H: int, N: int, x, x_prev, v_first, state, x_r, x_w, x_k, x_v, x_a, x_g, w0, w1,
                       w2, a0, a1, a2, v0, v1, v2, g1, g2, k_k, k_a, r_k, R_, K_, V_, O_, ln_w, ln_b):
    xx = x_prev - x
...
@MyStatic
def RWKV_x070_TMix_seq(layer_id: int, H: int, N: int, x, x_prev, v_first, state, x_r, x_w, x_k, x_v, x_a, x_g, w0, w1,
                       w2, a0, a1, a2, v0, v1, v2, g1, g2, k_k, k_a, r_k, R_, K_, V_, O_, ln_w, ln_b):
    T = x.shape[0]
    ...
    xx = torch.nn.functional.group_norm(xx.view(T, H * N), num_groups=H, weight=ln_w, bias=ln_b, eps=64e-5).view(T,
                                                                                                                 H * N)
