...
if "-h" in sys.argv or "--help" in sys.argv:
    print(
        f"Usage: python3 {sys.argv[0]} [--use-gpu] <base_model.pth> <lora_init.pth> <lora_checkpoint.pth> <output.pth>"
    )

if sys.argv[1] == "--use-gpu":
    device = "cuda"
    base_model, init_lora, lora, output = (
        sys.argv[2],
        sys.argv[3],
        sys.argv[4],
        sys.argv[5],
    ...
    for k in keys:
        if k.endswith(".weight"):
            ...
            if lora_A in keys:
                ...
                w[k] = (w[k] - w_init_lora[init_lora_B] @ w_init_lora[init_lora_A]).to(
