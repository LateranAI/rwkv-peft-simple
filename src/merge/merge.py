    ...
    for k in keys:
        if k.endswith(".weight"):
            ...
            if lora_A in keys:
                ...
                if args.type == "pissa":
                    w_init_lora[init_lora_A] = w_init_lora[init_lora_A].to(
                        device=device
                    )
                    w_init_lora[init_lora_B] = w_init_lora[init_lora_B].to(
                        device=device
                    )
                    if quant == "4bit":
                        qw, qs = bnb.functional.quantize_4bit(
                            w[k] - w_init_lora[init_lora_B] @ w_init_lora[init_lora_A]
                        )
                        w[k] = (bnb.functional.dequantize_4bit(qw, quant_state=qs)).to(
                            dtype=torch.bfloat16
                        )
                    elif quant == "nf4":
                        qw, qs = bnb.functional.quantize_nf4(
                            w[k] - w_init_lora[init_lora_B] @ w_init_lora[init_lora_A]
                        )
                        ...
                        qw, qs = bnb.functional.quantize_fp4(
                            w[k] - w_init_lora[init_lora_B] @ w_init_lora[init_lora_A]
                        )
